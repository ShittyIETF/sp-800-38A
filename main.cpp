...
int main() {
  ...
  for (int i = 0; i < 4 ; ++i) {
    Block b;
    b.read(&ECB_AES128_plaintext[i * 4 * Cipher<128>::block_size]);
    in.push_back(b);
  }
  ...
  for (int i = 0; i < 64; ++i) {
    printf("%02x",ECB_AES128_ciphertext[i]);
  }
  printf("\n");
  printf("Actual ciphertext:\n");
  for (int i = 0; i < 64; ++i) {
    printf("%02x",actualCiphertext[i]);
  }
  printf("\n");
